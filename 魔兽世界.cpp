...
public:	
	int type;  //武器种类
	virtual void strike(Warrior* owner, Warrior* enemy) {};
...
public:
	...
	{
		weap[0] = new Weapon(num % 3,this);
		weap[1] = new Weapon((num + 1) % 3,this);
...
void Warrior::sortWeap()
{
	...
	for (int i = 0; i < totalWeap; i++)
		...
		{
			weap[totalWeap++] = weap[i];
			weapNum[weap[i]->type] ++;
			weap[i] = 0;
		}
		else if (weap[i] != 0) delete weap[i];
	for (int i = 1; i < totalWeap; i++)
		for (int j = totalWeap - 1; j >= i; j--)
...
void City::grab() //wolf抢夺000:35 blue wolf 2 took 3 bomb from red dragon 2 in city 4
{
	...
	for (int i = warriors[bei]->weapNum[weapType]; warriors[lang]->totalWeap < 10 && i >= 0; i--)
	{
...
void City::fight()
{
	...
	int times = 0;
	while (warriors[0]->alive && warriors[1]->alive && times < 2000)
	{
		times++;
		while(warriors[0]->weap[redweap]->durability<=0) redweap = (redweap + 1) % warriors[0]->totalWeap;
		warriors[0]->weap[redweap]->strike(warriors[0], warriors[1]);
		redweap = (redweap + 1) % warriors[1]->totalWeap;
		while (warriors[1]->weap[redweap]->durability <= 0) redweap = (redweap + 1) % warriors[1]->totalWeap;
...
{
	weap[totalWeap] = new Weapon(Weap->type, this);
	weap[totalWeap]->durability = Weap->durability;
	weapNum[Weap->type] ++;
	totalWeap++;
}
...
{
	giveTime();
	cout << currHP << " elements in " << campColor << " headquarter" << endl;
}
Game::Game(int num)
{
	round = num;
